function fetchAndSummarizeSecurityArticles() {
  Logger.log('処理開始');
  
  // 1. 記事取得
  const articles = getYesterdaySecurityArticles();
  ...
  const fileName = `${today}_SecurityNewsSummary.txt`;
...
function saveDailySummaryToDrive(fileName, content) {
  ...
  const folderId = "YOUR_SECURITY_FOLDER_ID";
